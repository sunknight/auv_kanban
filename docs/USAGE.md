...
**A**gent **U**ser **V**antage · 基于 Markdown 文件 + 目录的人/智能体共用看板。
...
`kanban serve` 启动的 Web 服务按**本地单用户工具**定位，默认配置如下：
...
- 不要把监听地址改回 `0.0.0.0` 后接公网/不可信网络——一旦无认证服务被触达，看板数据可被任意读写。
- 在公司内网、公共 Wi-Fi 等环境运行时，保持默认的 `127.0.0.1` 绑定即可；同网段他人无法访问。

**如需远程访问**：请在前面加一层带鉴权的反向代理（如 nginx + Basic Auth / OAuth），由代理负责身份校验后再转发到本机服务，不要直接放开监听地址。
