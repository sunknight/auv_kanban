...
export const updateCommand = {
  ...
  describe: '对任务追加一条补充需求（写入 ## 子任务，带 [补充] 标签）并重开到 doing。默认追加后立即执行；加 --no-run 则只补需求、不立即执行',
  builder: (yargs: any) => yargs
    ...
    .option('run', {
      type: 'boolean',
      default: true,
      describe: '追加后立即执行刚追加的这条子任务（默认 true；传 --no-run 则只补需求不执行）',
    }),
  handler: async (argv: any) => {
    ...
    if (!text) {
      console.error('用法: kanban update [--no-run] <id> <需求文本>');
      process.exit(1);
    }
    ...
    if (argv.run) {
      // 默认行为：补了就做。明确告诉调用方（agent）现在就按 run 流程处理刚追加的编号。
      // 默认 run=true（补完就做）；传 --no-run 则走 else 分支（补了稍后做）。
      console.log(`已追加补充需求并重开，立即执行 → 子任务 ${no}`);
      console.log(`  本轮目标：${text}`);
      console.log(`  （按 SKILL.md run 流程：增量实现刚追加的 ${no}，完成后 kanban check ${id} ${no}）`);
    } else {
      // --no-run：只补需求、重开到 doing，等用户后续主动 /kanban run。
      console.log(`已追加补充需求并重开 → /kanban run ${id}`);
      console.log(`  新子任务编号 ${no}：${text}（完成执行后用 kanban check ${id} ${no} 勾上）`);
    }
  },
};
